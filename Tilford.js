(sand.define("layoutJS/Tilford", ["core/extend","layouts/Base"], function(r){
  ...
  *
  */
  var Tilford = function(options){
      r.Base.call(this,options);
  };
  Tilford.prototype = r.extend({},r.Base.prototype,{
      /*
      *  Structural
      *
      */
      ...
      compute: function(){
           ...
           if(mode === "circular"){
               ...
               for (var i = 0, l= root.mainChildren.length; i<l; i++){
                   trees.push(this._placeTree(root.mainChildren[i]));
               }
           }
           this._setPosition(trees[0]);
           ...
           if(this.posConfig.butterfly){
               for (var j = 0, n = tree[1].length; j <n; j++){
                    ...
                    for (var i = 2, n = tree.length; i <n; i++){
                          if(maxP < (tree[i].last()._d-tree[i].first()._d+this._getDistance(i))){
                              maxP = tree[i].last()._d-tree[i].first()._d+this._getDistance(i);
                          }
                    }
             ...
             for(var i = 0; i < this.vertices.length; i++){
               console.log(this.vertices[i]," yo yo yo");
               
               this.vertices[i].position = [this.vertices[i]._lvl,this.vertices[i]._d];
             }
           }
      },
    ...
    _placeTree: function(v) {
        var c = v.mainChildren, cs = [],maxLvl = 0;
        if (c.length) {
          for (var i = 0, n = c.length; i < n; i++) {
            cs.push(this._placeTree(c[i]));
            (c[i]._maxLvl > maxLvl) && (maxLvl = c[i]._maxLvl)
          }
          ...
          v._rect = [c.last()._rect[0]+c.last()._d,maxLvl];
          return ([[v]].concat(mrg));
        } else {
          v._d = 0;
          v._rect = [0,0];
      ...
      _mergeTrees: function(tree1, tree2) {
          ...
          for (var i = l; i--; ) {
            diff = tree1[i].last()._d - tree2[i].first()._d;
